                ...
                .then(function (data) {
                    weather.find({ search: data.city, degreeType: 'C' }, function (err, result) {
                        if (err) res.json({ "statusCode": 200, "body": "city not found" });
                            ...
                            // Create new tweet
                            client.post('statuses/update', { status: "It's " + result[0].current.observationtime + " in " + result[0].location.name + "and the temperature is " + result[0].current.temperature + result[0].location.degreetype }, function (error, tweet, response) {
